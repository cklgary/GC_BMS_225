...
```{r read in seur`at object with metadata}
...
```{r make and plot metadata before filtering}
#GC: below code remove data with high mitochondrial dna. how many percentage to set for threadshod? try and look at the end of the pipeline for the best one!
...
```{r filter for high quality cells and plot, fig.height=2.5, fig.width=5}
...
```{r plots comparing cells before and after filtration}
#visualizing the before and after plots is a easier way for your brain to keep track
...
# GC: these samples assignments are from lab notebook pipetting record 
# Add Tcelltype
...
Tcells@meta.data = Tcells@meta.data %>%
  unite('Tage', Tcelltype:Age, remove = FALSE) #remove = true will del the original column
...
#cell cycle is a very strong signature in scSeq, so there are packages to regress G2M and S phrase associated signals. however, list of genes to be regressed can depend on experiment setting and analysis required 
...
```{r filtered plots}
#one ? => look for topic in loaded package, ??=> load at all alllll r
multi_dittoPlot(object = Tcells,
                vars = c("nFeature_RNA", "nCount_RNA","percent.mito"),
                group.by = "orig.ident",
                plots = c("vlnplot","jitter"),
